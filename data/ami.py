...
def get_data_dict():
  ...
  if not os.path.exists(os.path.join(script_dir, "..", "ami", "ami_public_manual_1.6.2", "words")):
    raise FileNotFoundError(
      "AMI words not found at "
      + os.path.abspath(os.path.join(script_dir, "..", "ami", "ami_public_manual_1.6.2", "words"))
...
def extract_words(word_files):
  ...
  for word_file in word_files:
    with open(word_file, "r") as f:
      ...
      for child in root:
        if child.tag == "w" and "starttime" in child.attrib and "endtime" in child.attrib:
...
def create_text_from_words(words):
  ...
  for word in sorted_words:
    if last_word and last_word["speaker"] != word["speaker"] and word["is_punctuation"]:
      continue
...
def chunk_transcripts(wav_file_path, word_files):
  ...
  for i in range(0, int(length), 30):
    ...
    chunk_words = [word for word in words if word["start_time"] >= start and word["end_time"] <= end]

    text = create_text_from_words_with_speaker_change(chunk_words)

    chunks.append({"wav_file": wav_file_path, "start": start, "end": end, "text": text})
